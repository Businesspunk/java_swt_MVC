...
public class SearchEngine {
	...
	String eventName, sportName, fullName;

	Integer minCost, maxCost, minRev, maxRev;
	Date date;
	...
	protected boolean checkAll(Post item) {
		return checkEvent(item) && checkSportName( item ) && checkFullName(item) 
				&& checkAllCost( item ) && checkEarnCost( item ) && checkDate(item);
	}
